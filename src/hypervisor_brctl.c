...
/*
 * Create a veth pair (RTM_NEWLINK + IFLA_INFO_KIND="veth").
 * Both ends start DOWN; use brctl addif to attach one end to a bridge
 * and then bring it up with ip link set.
 * Returns 0 on success or a negative errno on failure (NOT -1).
 */
static int br_veth_pair(const char *name, const char *peer)
{
    struct nl_handler nlh;
    struct nlmsg *msg = NULL, *reply = NULL;
    struct ifinfomsg *ifi;
    ...
    /* Inside VETH_INFO_PEER: a zeroed struct ifinfomsg (raw, no NLA
     * header) followed by the peer interface attributes. */
    {
...
/* brctl veth <name> <peer> — create a veth pair */
static int cmd_veth(hypervisor_conn_t *conn, int argc, char *argv[])
{
    ...
    int err = br_veth_pair(name, peer);
    if (err < 0) {
